...
namespace simcluster_utils {
  ...
  std::vector<DetId::Detector> join_detids(const std::vector<std::string>& dets_v) {
    ...
    for (const auto& det : dets_v) {
      ...
      // std::cout << "AFTER DetId INSERT: " << det << ", " << detIds.empty() << ", " << detIds.size() << ", " << *std::min_element(detIds.begin(), detIds.end()) << ", " << *std::max_element(detIds.begin(), detIds.end()) << std::flush << std::endl;
    }
    return detIds;
  }
